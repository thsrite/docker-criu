...
class cloudlet_handler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
		...
		msg = "images:"
            	if self.recv_file(self.task_id + '-image.tar', int(str_array[3])):
                    msg += "sucess"
                else:
                    msg += "failed"
                self.send_msg(msg)

            	logging.info("get docker images msg success\n")
		rstore_handle.init_docker_images(self.task_id, self.lable, str_array[4])
...
class daemon:
    ...
    def run(self):
        host = ni.ifaddresses('enp0s5')[2][0]['addr']
