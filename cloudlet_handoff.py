...
class handoff:
    ...
    #---step2: send docker images:
	mm_handle = cloudlet_memory(self.task_id)
	label_ar = self.label.split('-')
        con_name = label_ar[0]
        base_img = label_ar[1]
        img_id = label_ar[2]
	images_path = mm_handle.images_path()
	save_op = 'docker export  ' + con_name + ' -o ' + images_path
	print(save_op)
	ret = sp.call(save_op, shell=True)
	logging.info(ret)

        if ret != 0:
            logging.error('docker images save failed')
            return False
	print('docker images save success')
	cmd = '"'
	for i in range(len(self.cmd)):
	    if i == 0:
                cmd = cmd + str(self.cmd[i])
	    else:
	        cmd = cmd + ' ' + str(self.cmd[i])
	cmd = cmd + '"'
	msg_fs = 'images#' + self.task_id + '#' + self.label + '#' + str(os.path.getsize(images_path)) + '#' + cmd + '#'
        clet_socket.send(msg_fs)
        clet_socket.send_file(images_path)
        data = clet_socket.recv()

        logging.debug('start send docker images file ....')
    ...
    #---step3: predump:
        ...
        while(start_predump):
            ...
            premm_img = mm_handle.mm_img_path()
